...
def bake(map,frame):   
    ...
    if not selected_to_active:        
        img.filepath = "%s/%s_%s%s.exr" % (imgpath, object.name, map.replace("_",""),"%04d" % (frame)  if getattr(bpy.context.scene.bake_pbr_channels,"sequence") else "")
    else:
        img.filepath = "%s/%s_%s%s.exr" % (imgpath, object.active_material.name, map.replace("_",""),"%04d" % (frame)  if getattr(bpy.context.scene.bake_pbr_channels,"sequence") else "")   
